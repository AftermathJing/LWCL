...
class RotaryEncoderLayer(nn.Module):
    def __init__(self, d_model: int, num_heads: int, ffn_dim: int, dropout: float, rotary_dim: int | None) -> None:
        super().__init__()
...
class RotaryEncoder(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [RotaryEncoderLayer(d_model, num_heads, ffn_dim, dropout, rotary_dim) for _ in range(num_layers)]
        )
